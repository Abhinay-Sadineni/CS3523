...
void *threak_work(void *p)
{
  ...
  for (int i = 0; i < iter; i++)
  {
    ...
    // entry section
    bool expected = false;
    while (!lock.compare_exchange_strong(expected, true, std::memory_order_seq_cst))
    {
      expected = false;
    }
...
int main(int argc, char const *argv[])
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    p->t2 = rng_2(gen);
    p->id = i;
    pthread_create(&tid[i], NULL, threak_work, (void *)p);
  }
  ...
  //log generation
  fprintf(fp, "CAS ME Output(format:min/sec/millisec):\n\n");
